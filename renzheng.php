	...
	if($_FILES){
	 ...
	 if(!move_uploaded_file($_FILES['upfile']['tmp_name'][0],$picName1)){
	   ...
	   $picname4 = $CFG['url'].$ddd2 = substr($picName2,2);
	    $db->query("INSERT INTO {$AJ_PRE}validate (type,username,ip,addtime,status,editor,edittime,title,thumb,thumb1,thumb2) VALUES ('truename','$_username','$AJ_IP','$AJ_TIME','2','system','$AJ_TIME','$_username','$picname3','$picname4','')");
